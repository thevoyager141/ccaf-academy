  ...
  { id:"p1c1", ch:"CH 1", title:"에이전트 루프의 생명주기 (1.1)",
    steps:[
      ...
      {type:"concept", kind:"CONCEPT · 개념 설명", h:"재료부터: 대화 히스토리와 stop_reason",
       html:`<p class="lead">이 레슨에 필요한 용어는 두 개. 그 전에 코스 전체의 바탕 용어부터.</p>
        <div class="callout">📖 용어 카드 — <b>토큰</b>: 모델이 글을 세는 단위(대략 단어 조각 하나). 분량·비용·속도가 전부 토큰 수로 계산돼. <b>컨텍스트(창)</b>: 모델이 지금 한 번에 보고 있는 작업 기억 전체(이번 대화 + 받은 자료). 창 크기 = 한 번에 담을 수 있는 토큰 한도. 이 두 단어는 이후 모든 파트에서 그냥 등장하니 여기서 잡아두자.</div>
        <h4>① 대화 히스토리</h4>
        <ul>
    ...
    steps:[
      {type:"concept", kind:"CONCEPT · 개념 설명", h:"일을 쪼개는 두 가지 방식",
       html:`<p class="lead">큰 작업을 에이전트에게 시킬 때, 쪼개는 방식은 크게 둘.</p>
        <h4>① Prompt chaining — 고정 순차 파이프라인 (파이프라인 = 정해진 순서로 이어지는 작업 라인, 공장 컨베이어)</h4>
        <ul>
        ...
        <ul>
          ...
          <li>예: 조사하다가 의존성(A를 고치려면 B도 건드려야 하는 연결 관계)을 발견하면 계획에 새 하위 작업 추가</li>
      ...
      {type:"concept", kind:"TOPIC · 주제 설명", h:"어떤 작업에 어떤 분해를 쓰나",
       html:`__MAP:decomp__<p class="lead">시험은 "이 작업엔 어느 쪽?"을 물어본다. 판별 기준과 대표 케이스 두 개.</p>
        ...
        <h4>케이스 2: "레거시 코드(오래돼 문서도 담당자도 없는 기존 코드)에 테스트 전면 추가" (동적 분해의 대표 예)</h4>
        <ul>
