...
window.CCAF_CONTENT.p4 = {
  ...
  { id:"p4c1", ch:"CH 1", title:"명시적 기준으로 정밀도 올리기 (4.1)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — The distrusted reviewer",
       q:`Your automated code reviewer has an 80% false positive rate on style findings, and developers have begun ignoring its accurate security findings too. Adding "only report high-confidence issues" to the prompt didn't help. What is the most effective fix?`,
       opts:[
         {t:`Define explicit categorical criteria — report bugs and security issues, skip minor style and local patterns — and temporarily disable the style category while its prompt is improved.`, ok:true},
         {t:`Strengthen the instruction to "be extremely conservative and only report definite issues."`},
         {t:`Add a confidence score threshold of 0.9 for all findings.`},
         {t:`Reduce the reviewer's scope to security findings permanently.`}],
       hint:`"confidence 기반 지시는 이미 실패했다"고 지문에 나와 있어. 남은 축은 뭐지? 그리고 신뢰 회복을 위한 임시 조치도 가이드에 있었지.`,
       explain:{
         good:`가이드 4.1의 두 처방 결합: 범주형 기준(보고 vs 스킵) + 오탐 범주 일시 비활성화로 신뢰 복구. "신중히" 계열은 판단선을 못 만들어.`,
         wrongs:[
           `<b>B:</b> 이미 실패한 접근의 강화판 — 기준 없는 신중함은 없어.`,
           `<b>C:</b> 자기 보고 신뢰도는 보정되지 않음 — 숫자만 그럴듯한 같은 실패.`,
           `<b>D:</b> 영구 축소는 스타일 리뷰 가치를 포기 — 일시 비활성화 + 개선이 정답.`,
         ]},
       principle:"기준은 범주로, 신뢰는 격리로 복구"},
  ...
  { id:"p4c2", ch:"CH 2", title:"few-shot 프롬프팅 (4.2)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — Teaching the gray zone",
       q:`Your support agent handles clear refund requests well, but on ambiguous messages ("this product isn't what I expected...") it inconsistently chooses between offering a return, troubleshooting, or escalating. Detailed routing instructions haven't stabilized it. What is the most effective addition?`,
       opts:[
         {t:`2-4 few-shot examples of ambiguous messages, each showing the chosen action and the reasoning for why it beat the plausible alternatives.`, ok:true},
         {t:`A longer decision tree in prose covering every message type you can enumerate.`},
         {t:`10+ examples of clear-cut refund requests being handled correctly.`},
         {t:`A rule to always escalate any message classified as ambiguous.`}],
       hint:`예시는 몇 개, 어떤 케이스로, 뭘 포함해서? 세 가지 다 가이드에 명시돼 있어.`,
       explain:{
         good:`가이드 4.2의 공식: 애매한 케이스 2~4개 + 선택 이유 포함. 이유가 있어야 판단 기준이 새 패턴으로 일반화돼.`,
         wrongs:[
           `<b>B:</b> 열거식 트리는 못 본 케이스에서 무너져 — 일반화가 없어.`,
           `<b>C:</b> 명백한 케이스 예시는 이미 잘하는 걸 더 가르치는 낭비.`,
           `<b>D:</b> 애매하면 전부 인간에게 — 자율 해결률 붕괴 (5.2에서 다룰 과잉 에스컬레이션).`,
         ]},
       principle:"애매 케이스 + 이유 = 일반화"},
  ...
  { id:"p4c3", ch:"CH 3", title:"tool_use와 JSON 스키마로 출력 강제 (4.3)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — The fabricated dates",
       q:`Your invoice extractor's schema marks purchase_order_date as required. For invoices that simply don't contain a PO date, the model outputs plausible-looking fabricated dates instead of leaving the field empty. What is the correct schema fix?`,
       opts:[
         {t:`Make purchase_order_date optional (nullable), so the model can return null when the source document lacks the information.`, ok:true},
         {t:`Keep it required but add a prompt instruction: "never fabricate dates."`},
         {t:`Add a second required field, date_confidence, to flag uncertain dates.`},
         {t:`Post-process extracted dates and discard any that look suspicious.`}],
       hint:`모델이 지어낸 이유는 "빈칸을 허용하지 않는 양식" 때문이야. 양식 쪽을 고치면?`,
       explain:{
         good:`required 필드 + 정보 부재 = 환각 제조기. nullable로 바꾸면 모델이 정직하게 null을 낼 수 있어 — 가이드 4.3의 핵심 설계 원칙.`,
         wrongs:[
           `<b>B:</b> 구조가 지어내기를 강요하는데 지시로 막는 건 확률적 — 구조 vs 지시의 싸움에선 구조가 이겨.`,
           `<b>C:</b> 지어낸 날짜에 신뢰도 라벨을 붙여봤자 지어낸 날짜야.`,
           `<b>D:</b> "그럴듯하게" 지어낸 값은 의심스러워 보이지 않아 — 탐지 불가.`,
         ]},
       principle:"없을 수 있으면 nullable — 구조로 환각 차단"},
  ...
  { id:"p4c4", ch:"CH 4", title:"검증·재시도·피드백 루프 (4.4)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — The useful failure",
       q:`Pydantic validation rejects an extraction because "line_items[2].amount" is a string ("$1,200.00") instead of a number. How should the retry request be constructed?`,
       opts:[
         {t:`Send a follow-up request containing the original document, the failed extraction, and the specific validation error, so the model can correct the exact problem.`, ok:true},
         {t:`Resend the original request unchanged — validation failures are usually transient.`},
         {t:`Send only the failed extraction with the message "fix the errors in this output."`},
         {t:`Switch to a different model for retry attempts, since this one produced an error.`}],
       hint:`재시도에 동봉하는 3종 세트가 뭐였지?`,
       explain:{
         good:`가이드 4.4의 재시도 공식: 원본 + 실패 출력 + 구체적 오류. 모델이 정확히 뭘 고칠지 알고, 형식 오류라 재시도가 통하는 케이스야.`,
         wrongs:[
           `<b>B:</b> 같은 입력 = 같은 실수 확률. 검증 실패는 무작위 장애가 아니야.`,
           `<b>C:</b> 원본 없이 "고쳐"라면 뭐가 맞는 값인지 알 수 없어.`,
           `<b>D:</b> 형식 실수 하나로 모델 교체 — 과잉 대응이고 재발 방지도 아니야.`,
         ]},
       principle:"재시도 = 원본 + 실패 + 오류 명세"},
  ...
  { id:"p4c5", ch:"CH 5", title:"배치 처리 전략 — Batches API (4.5)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — Two workflows, one proposal",
       q:`Real-time Claude calls currently power (1) a blocking pre-merge check developers wait on, and (2) an overnight technical-debt report read the next morning. Your manager proposes moving both to the Batches API for the 50% savings. How should you evaluate this?`,
       opts:[
         {t:`Move only the overnight report to batch processing; keep real-time calls for the pre-merge check.`, ok:true},
         {t:`Move both — batches usually complete quickly, so developers rarely notice.`},
         {t:`Move neither — batch result ordering issues make it unreliable for reports.`},
         {t:`Move both, adding a timeout fallback to real-time when batches run long.`}],
       hint:`두 워크플로의 결정적 차이: 사람이 결과를 기다리는가? 그리고 배치엔 "언제까지"라는 약속이 없었지.`,
       explain:{
         good:`배치는 24시간까지 걸릴 수 있고 SLA가 없어 — 블로킹(머지 전 검사)엔 부적합, 야간 리포트엔 이상적 (V2 샘플 11번).`,
         wrongs:[
           `<b>B:</b> "보통 빠르다"는 보장이 아니야 — 블로킹 흐름에 도박 금지.`,
           `<b>C:</b> 순서 문제는 custom_id로 해결돼 — 잘못된 전제.`,
           `<b>D:</b> 폴백 이중화는 단순 분리로 될 일을 복잡하게 — 과잉 설계.`,
         ]},
       principle:"블로킹 = 실시간, 논블로킹 = 배치"},
  ...
  { id:"p4c6", ch:"CH 6", title:"다중 인스턴스·다중 패스 리뷰 (4.6)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — Reviewing its own work",
       q:`Your pipeline has Claude generate a code change, then asks the same session to "carefully review your work for subtle bugs before finalizing." The reviews consistently approve code that later fails in production. What is the structural fix?`,
       opts:[
         {t:`Have a second, independent Claude instance review the generated code — without the generator's reasoning context, it can genuinely question the decisions.`, ok:true},
         {t:`Strengthen the self-review instruction: "assume the code contains at least three bugs and find them."`},
         {t:`Enable extended thinking during the self-review step for deeper analysis.`},
         {t:`Have the same session review the code twice before finalizing.`}],
       hint:`문제는 지시의 강도가 아니라 세션에 남아 있는 '무엇'이야.`,
       explain:{
         good:`생성 세션의 추론 맥락이 자기 결정 옹호로 작동하는 게 원인 — 맥락 없는 독립 인스턴스만이 구조적 해법 (가이드 4.6).`,
         wrongs:[
           `<b>B:</b> 버그 가정 강제는 억지 오탐을 만들거나, 여전히 자기 논리 안에서 '찾는 척'하게 돼.`,
           `<b>C:</b> extended thinking도 같은 맥락 안의 사고 — 더 깊이 자기를 설득할 뿐.`,
           `<b>D:</b> 같은 세션 두 번 = 같은 편향 두 번.`,
         ]},
       principle:"자기 리뷰의 한계 = 맥락 — 독립 인스턴스로"},
