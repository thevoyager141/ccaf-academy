...
window.CCAF_CONTENT.p5 = {
  ...
  { id:"p5c1", ch:"CH 1", title:"긴 대화에서 핵심 정보 보존 (5.1)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — The vanishing refund amount",
       q:`In long support conversations, your agent summarizes older turns to stay within context limits. Customers complain it "forgets" agreed refund amounts and promised dates — the summaries say things like "discussed refund options." What is the best fix?`,
       opts:[
         {t:`Extract transactional facts (amounts, dates, order numbers, statuses) into a persistent "case facts" block that is excluded from summarization and included in every prompt.`, ok:true},
         {t:`Instruct the summarizer to "keep all important details" when compressing turns.`},
         {t:`Stop summarizing entirely and let the conversation truncate naturally from the top.`},
         {t:`Increase the context window so summarization starts later in the conversation.`}],
       hint:`요약이라는 압축기에 숫자를 넣으면 뭉개져. 그럼 숫자는 어디에 보관해야 할까?`,
       explain:{
         good:`가이드 5.1의 핵심 패턴: 거래 사실은 요약 파이프라인 밖의 구조화 블록에 — 압축을 거치지 않으니 뭉개질 수 없어.`,
         wrongs:[
           `<b>B:</b> "중요한 건 지켜"는 확률적 — 무엇이 중요한지의 정의도 없어.`,
           `<b>C:</b> 앞부분 절삭은 초기 합의(금액 약속)를 통째로 잃는 더 나쁜 손실.`,
           `<b>D:</b> 시작만 늦출 뿐 — 충분히 긴 대화에선 같은 문제 재발.`,
         ]},
       principle:"숫자는 요약 밖 금고에 — case facts"},
  ...
  { id:"p5c2", ch:"CH 2", title:"에스컬레이션과 모호성 해결 (5.2)",
    steps:[
      ...
      {type:"concept", kind:"TOPIC · 주제 설명", h:"판단 케이스 4종 세트",
       html:`__MAP5:esc__<h4>케이스 ①: 55% 해결률, 거꾸로 된 에스컬레이션 (샘플 3번)</h4>
        <ul>
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — Upside-down escalation",
       q:`Your agent achieves 55% first-contact resolution against an 80% target. Logs show it escalates straightforward cases (standard damage replacements with photo evidence) while attempting policy-exception cases autonomously. What most effectively fixes the calibration?`,
       opts:[
         {t:`Add explicit escalation criteria to the system prompt with few-shot examples demonstrating when to escalate versus resolve autonomously.`, ok:true},
         {t:`Have the agent self-report confidence and auto-route below a threshold.`},
         {t:`Deploy a classifier model trained on historical tickets to predict escalation needs.`},
         {t:`Add sentiment analysis and escalate when customer frustration exceeds a threshold.`}],
       hint:`문제는 판단선이 없다는 것. 그리고 이 에이전트는 이미 어려운 케이스에서 '잘못된 확신'을 보이고 있지 — 그럼 자기 신뢰도는?`,
       explain:{
         good:`근본 원인은 불명확한 결정 경계 — 명시 기준 + few-shot이 비례에 맞는 첫 처방 (V2 샘플 3번).`,
         wrongs:[
           `<b>B:</b> 이미 어려운 케이스에서 과신 중 — 자기 보고 신뢰도는 보정 안 된 대리 지표.`,
           `<b>C:</b> 프롬프트 최적화도 안 해보고 ML 인프라부터 — 과잉 설계.`,
           `<b>D:</b> 감정은 난이도와 상관없어 — 다른 문제를 푸는 보기.`,
         ]},
  ...
  { id:"p5c3", ch:"CH 3", title:"멀티에이전트 에러 전파 (5.3)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — The timeout report",
       q:`Your web search subagent times out while researching a complex topic. How should the failure flow back to the coordinator to enable intelligent recovery?`,
       opts:[
         {t:`Return structured error context: the failure type, the attempted query, any partial results, and potential alternative approaches.`, ok:true},
         {t:`Retry internally with exponential backoff, returning a generic "search unavailable" status only after retries are exhausted.`},
         {t:`Catch the timeout and return an empty result set marked as successful.`},
         {t:`Propagate the exception to a top-level handler that terminates the research workflow.`}],
       hint:`코디네이터가 다음 수를 두려면 뭘 알아야 할까? 4요소를 떠올려.`,
       explain:{
         good:`4요소가 코디네이터의 선택(수정 재시도/대안/부분 진행)을 가능하게 해 (V2 샘플 8번). 내부 재시도 자체는 좋지만 소진 후에도 구조화 컨텍스트가 가야 해.`,
         wrongs:[
           `<b>B:</b> 백오프는 좋은데 "unavailable" 한 마디가 모든 맥락을 버려 — 반쪽 정답이 함정.`,
           `<b>C:</b> 실패의 성공 위장 — 복구 기회 자체를 없애는 최악수.`,
           `<b>D:</b> 복구 전략이 있는데 전체 종료 — 과잉 반응.`,
         ]},
       principle:"원칙 ⑤ 실패는 4요소로"},
  ...
  { id:"p5c4", ch:"CH 4", title:"대형 코드베이스 탐색의 컨텍스트 관리 (5.4)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — The fading memory",
       q:`Three hours into exploring a large codebase, Claude starts answering with "typically, authentication services follow this pattern..." instead of referencing the specific AuthGateway class it analyzed earlier. What is the best countermeasure?`,
       opts:[
         {t:`Have the agent maintain scratchpad files recording key findings as it works, and reference them for subsequent questions.`, ok:true},
         {t:`Restart the session and re-explore the codebase from scratch.`},
         {t:`Remind the agent in each prompt to "use only what you actually found."`},
         {t:`Reduce the exploration scope so the session stays short.`}],
       hint:`증상은 컨텍스트 열화야. 대화 '밖'에 기억을 두는 방법이 있었지?`,
       explain:{
         good:`"일반적 패턴" 화법은 열화의 시그니처 — 스크래치패드 파일이 대화 밖에서 구체 발견을 보존해 (가이드 5.4).`,
         wrongs:[
           `<b>B:</b> 재탐색은 비용 재지불 + 새 세션도 다시 열화돼.`,
           `<b>C:</b> 잊어버린 걸 쓰라는 지시 — 없는 기억은 소환 안 돼.`,
           `<b>D:</b> 범위 축소는 과제 포기 — 대형 코드베이스가 요구사항인데.`,
         ]},
       principle:"기록이 기억을 대신한다 — 스크래치패드"},
  ...
  { id:"p5c5", ch:"CH 5", title:"인간 리뷰 워크플로와 신뢰도 보정 (5.5)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — The 97% trap",
       q:`Your extraction system shows 97% overall accuracy, and management wants to eliminate human review for high-confidence extractions. What must happen before automating?`,
       opts:[
         {t:`Analyze accuracy by document type and field segment to verify consistent performance across all segments — aggregate accuracy can mask poor performance on specific types.`, ok:true},
         {t:`Proceed — 97% exceeds any reasonable threshold for automation.`},
         {t:`Raise the overall accuracy to 99% first, then automate everything.`},
         {t:`Automate immediately but keep a customer complaint channel to catch errors.`}],
       hint:`97%는 평균이야. 평균이 숨길 수 있는 게 뭐였지?`,
       explain:{
         good:`집계 지표는 세그먼트 저성능을 가려 — 유형별·필드별 분해 검증이 자동화의 전제 조건 (가이드 5.5).`,
         wrongs:[
           `<b>B:</b> 평균 뒤에 70% 유형이 숨어 있으면 그 유형은 계통적으로 오염돼.`,
           `<b>C:</b> 99%도 평균이야 — 같은 함정의 더 높은 버전.`,
           `<b>D:</b> 고객을 오류 탐지기로 쓰는 설계 — 신뢰 비용이 훨씬 커.`,
         ]},
       principle:"평균을 쪼개라 — 세그먼트 검증"},
  ...
  { id:"p5c6", ch:"CH 6", title:"출처 보존과 불확실성 처리 (5.6)",
    steps:[
      ...
      {type:"quiz", kind:"PRACTICE · 영어 출제", h:"Practice 1 — Two credible numbers",
       q:`During document analysis, two credible sources give different figures for the same market size ($4.2B from a 2023 industry report, $5.1B from a 2024 analyst note). How should the analysis agent handle this before passing results downstream?`,
       opts:[
         {t:`Complete the analysis with both values included and explicitly annotated with their sources and dates, letting the coordinator decide how to reconcile before synthesis.`, ok:true},
         {t:`Select the more recent figure, since newer data supersedes older data.`},
         {t:`Average the two figures to produce a single balanced estimate.`},
         {t:`Omit the metric entirely, since conflicting data is unreliable.`}],
       hint:`분석 에이전트의 역할 경계를 생각해봐 — 충돌을 '해결'하는 게 이 단계의 일일까?`,
       explain:{
         good:`가이드 5.6: 충돌 값은 출처·시점 주석과 함께 포함한 채 분석을 완료하고, 조정 권한은 코디네이터에 — 하류가 정보를 잃지 않아.`,
         wrongs:[
           `<b>B:</b> 최신 우선은 그럴듯한 휴리스틱이지만 방법론 차이일 수 있어 — 임의 선택 금지.`,
           `<b>C:</b> $4.65B는 어느 소스도 말한 적 없는 발명된 숫자.`,
           `<b>D:</b> 충돌은 정보야 — 지우면 리포트가 그 불확실성을 알릴 기회도 사라져.`,
         ]},
       principle:"고르지 말고 둘 다, 주석 달아서"},
